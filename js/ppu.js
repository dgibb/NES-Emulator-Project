		...
		case 1:
			...
			case 4:
				if (ppu.cycle===340){ppu.mode=0;
					 ppu.cycle=-1;
					 //if (ppu.graphicsDebug){console.log('end scanline', ppu.scanline);}
...
fetchSpriteLow:function(){
	ppu.spriteTileOffset=oamBuf[ppu.spritesFound][1]<<3;
	ppu.spriteTileOffset+=(ppu.scanline+1)-oamBuf[ppu.spritesFound][0]
	ppu.spriteShiftReg[ppu.spritesFound][0]=ppu.readVRam(ppu.spriteTableOffset+ppu.spriteTileOffset);
	ppu.spriteTileOffset+=8;

},

fetchSpriteHigh:function(){
	ppu.spriteShiftReg[ppu.spritesFound][1]=ppu.readVRam(ppu.spriteTableOffset+ppu.spriteTileOffset);
	ppu.spritesFound++;
	ppu.spritesFound%=8;
},
		...
		if((ppu.cycle-1)>=ppu.spriteX[i]&&(ppu.cycle-1)<(ppu.spriteX[i]+8)){//x coord is in range
			...
			pix+=((ppu.spriteShiftReg[i][1]>>(7-shift))&0x01)?2:0;
				if(pix!==0){		//pix!=0 put image data
					...
					pixData.data[ppu.canvasOffset-3]=palette[color][0];
					pixData.data[ppu.canvasOffset-2]=palette[color][1];
					pixData.data[ppu.canvasOffset-1]=palette[color][2];
				}
			}
		}
},
				...
				switch(ppu.srCase2Mode){
					...
					case 1:	//loading and checking each sprite
						if(!(ppu.cycle%2)){
							...
							if ((oamBuf[ppu.spritesFound][0]>=(ppu.scanline+1))&&(oamBuf[ppu.spritesFound][0]<(ppu.scanline+9))){
								ppu.srCase2Mode=2;
	...
	switch(addr&0xF000){
		...
		case 0x2000:
			...
			switch(addr&0xF00){
				...
				case 0:
				return nameTable[0][addr-0x2400];
				break
...
oamDMA:function(addr){
	console.log('DMA', (addr<<8).toString(16));
	addr<<=8;
